...
data = np.loadtxt("/Users/mikkelodeon/optomechanics/Double fano cavity/M3+M5/data/20250326/21um/fsr.txt")
PI = np.loadtxt("/Users/mikkelodeon/optomechanics/Double fano cavity/M3+M5/data/20250326/21um/fsr_PI.txt")
...
plt.scatter(data[:,0], data[:,1], marker=".", color="maroon", label="data")
plt.plot(xs, fabry_perot(xs, *popt), color="orangered", alpha=0.7, label="$l \\approx$%5.2f$\\pm$%5.2fμm" % tuple(length)) 
